...
- **Coverage rebalanced across all 8 active methodologies**, not just WSTG — the View-1003 push initially concentrated almost everything into WSTG/ISSAF/NIST. Added domain-appropriate follow-up CWEs to the others: MASTG 27→34→**41**, OSSTMM 6→**12** (doubled — one per channel had zero CWE-backed vulnerability before), PTES 14→**19**, OWASP IoT 12→16→**26**, ICS/OT 9→**16** (segmentation exposure, unauthenticated control commands, HMI credential storage, untrusted PLC logic, single-factor remote access).
- **Sequential per-methodology deep dive**, one methodology at a time rather than one big batch:
  - **OWASP IoT / ICS-OT**: cross-checked against MITRE's **CWE View-1194** (Hardware Design, 145 members) and hand-picked the ~12 genuinely externally-pentestable ones (side-channel analysis, debug-interface data leakage, missing hardware root of trust, non-updateable firmware, insecure device decommissioning, shared unlock credentials, EM fault injection / voltage-glitching against controllers) — deliberately skipped the bulk of that view, which is silicon/SoC-design-team territory, not pentest-actionable.
  - **OWASP MASTG**: cross-checked against OWASP's newer **MASWE catalog** (Mobile App Security Weakness Enumeration, ~78 entries, supersedes the old numbered MASTG test list) and added 7 CWEs for gaps it covers that the old MASTG numbering didn't (TLS hostname-mismatch validation, embedded malicious third-party SDK code, missing app/resource integrity verification, unauthenticated exported components, tapjacking/overlay attacks, unrestricted keystore access, unsafe untrusted-data handling).
  - **OWASP WSTG**: confirmed there's no separate official WSTG→CWE mapping document beyond what the guide's own test names already imply (already reflected at 111 CWEs, by far the deepest methodology here) — no further gain found.
  - **PTES / OSSTMM / NIST SP800-115 / ISSAF**: confirmed these remain process/phase frameworks with no CWE body of their own (as already noted); checked specifically for a dedicated wireless-auth CWE (WPA/WEP) since OSSTMM/NIST both call out wireless testing explicitly — none exists beyond the weak-crypto/weak-credential CWEs already in the catalog (verified via CWE-1391's parent/child hierarchy on cwe.mitre.org).
...
17. Sequential deep-dive per methodology: IoT/ICS-OT against MITRE's CWE View-1194 (hardware), MASTG against OWASP's newer MASWE catalog; confirmed WSTG/PTES/OSSTMM/NIST/ISSAF have no further official CWE source to mine beyond what's already covered. Catalog now at 189 distinct CWEs overall.
